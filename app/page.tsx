      ...
      <div className="flex flex-col w-[60%] border-r border-slate-200 overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-slate-200 bg-white">
          <div className="flex items-center justify-between mb-3">
            <div>
              <h2 className="text-2xl font-bold text-slate-900">Watch Inventory</h2>
              <p className="text-slate-400 text-sm mt-0.5">{watches.length} active watches</p>
            </div>
            <button
              onClick={() => setShowAddWatch(true)}
              className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-semibold transition-colors text-base"
            >
              ...
              return (
                <div key={dept} className={`rounded-xl px-3 py-2 border ${cfg.bg} ${cfg.border} flex items-center gap-2`}>
                  ...
                  <div>
                    ...
                    <div className="text-slate-900 font-bold text-lg leading-none">{stageCounts[dept]}</div>
                  </div>
                </div>
              )
            })}
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-slate-50">
          {watches.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-slate-400 gap-4">
              ...
              <button onClick={() => setShowAddWatch(true)} className="text-blue-600 hover:underline text-base">
      ...
      <div className="flex flex-col w-[40%] overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-white">
          ...
          <div className="flex items-center gap-2 text-sm">
            <span className={`w-2 h-2 rounded-full ${sseConnected ? 'bg-green-500 live-dot' : 'bg-yellow-400'}`} />
            <span className="text-slate-400">{sseConnected ? 'Live' : 'Polling'} · {secsAgo}s ago</span>
          </div>
        </div>

        {/* Auto-scrolling task list grouped by department */}
        <AutoScrollList className="flex-1 bg-slate-50" speedPxPerSec={40}>
          <div className="p-5">
            {members.length === 0 ? (
