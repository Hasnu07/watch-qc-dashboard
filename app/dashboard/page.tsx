...
const DEPT_CONFIG = {
  LOGISTICS: { label: 'Logistics', icon: '📦', color: 'text-blue-700', bg: 'bg-blue-50 border border-blue-100', countColor: 'text-blue-900', solid: 'bg-blue-500' },
  ACCOUNTING: { label: 'Accounting', icon: '💰', color: 'text-amber-700', bg: 'bg-amber-50 border border-amber-100', countColor: 'text-amber-900', solid: 'bg-amber-500' },
  SALES: { label: 'Sales', icon: '🤝', color: 'text-emerald-700', bg: 'bg-emerald-50 border border-emerald-100', countColor: 'text-emerald-900', solid: 'bg-emerald-500' },
} as const
...
export default function DashboardPage() {
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      ...
      {/* Mobile tab bar */}
      <div className="flex md:hidden border-b border-[#dee2ef] bg-white sticky top-0 z-40">
        <button onClick={() => setActiveTab('inventory')}
          className={`flex-1 py-3 text-sm font-bold transition-colors flex items-center justify-center gap-1.5 ${activeTab === 'inventory' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-[#777587]'}`}>
          ⌚ Inventory
          <span className={`text-xs px-1.5 py-0.5 rounded-full font-black ${activeTab === 'inventory' ? 'bg-indigo-50 text-indigo-600' : 'bg-[#eaeefb] text-[#777587]'}`}>{watches.length}</span>
        </button>
        <button onClick={() => setActiveTab('tasks')}
          className={`flex-1 py-3 text-sm font-bold transition-colors flex items-center justify-center gap-1.5 ${activeTab === 'tasks' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-[#777587]'}`}>
          ✅ Buy Tasks
        </button>
        <button onClick={() => setActiveTab('sell')}
          className={`flex-1 py-3 text-sm font-bold transition-colors flex items-center justify-center gap-1.5 ${activeTab === 'sell' ? 'text-orange-600 border-b-2 border-orange-600' : 'text-[#777587]'}`}>
          🏷️ Sold
        </button>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className={`flex-col w-full md:w-[58%] border-r border-[#dee2ef] overflow-hidden ${activeTab === 'inventory' ? 'flex' : 'hidden'} md:flex`}>
          ...
          <div className="px-4 py-4 border-b border-[#dee2ef] bg-white sm:px-6 sm:py-5">
            <div className="flex items-center justify-between mb-3 sm:mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setShowPasteMessage(true)}
                  ...
                  className="flex items-center gap-1.5 px-3 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-full font-bold transition-all text-sm sm:px-4 sm:py-2.5">
                  <span className="text-base">📋</span>
                  <span className="hidden sm:inline">Paste</span>
                </button>
                <button onClick={() => setShowAddWatch(true)}
                  className="flex items-center gap-1.5 px-3 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-full font-bold transition-all text-sm sm:gap-2 sm:px-5 sm:py-2.5 sm:text-base">
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-3">
              {DEPT_ORDER.map(dept => {
                ...
                return (
                  <div key={dept} className={`${cfg.bg} rounded-xl px-2.5 py-2 flex items-center gap-2 sm:rounded-2xl sm:px-4 sm:py-3 sm:gap-3`}>
                    ...
                    <div>
                      ...
                      <div className={`font-black text-xl leading-none sm:text-2xl ${cfg.countColor}`}>{stageCounts[dept]}</div>
                    </div>
                  </div>
                )
              })}
        ...
        <div className={`flex-col w-full md:w-[42%] overflow-hidden ${activeTab === 'tasks' || activeTab === 'sell' ? 'flex' : 'hidden'} md:flex`}>
          ...
          <div className="flex items-center justify-between px-4 py-4 border-b border-[#dee2ef] bg-white sm:px-6 sm:py-5">
            <div>
              <h2 className="text-xl font-black text-[#171c25] sm:text-2xl">Watch Tasks</h2>
          ...
          <div className="flex border-b border-[#dee2ef] bg-white px-4 gap-1 pt-2">
            <button
              onClick={() => { setTaskTab('buy'); setActiveTab('tasks') }}
              className={`px-4 py-2 text-sm font-bold rounded-t-lg transition-colors border-b-2 ${taskTab === 'buy' ? 'border-indigo-600 text-indigo-700' : 'border-transparent text-[#777587] hover:text-[#464555]'}`}>
              🛒 Buy Tasks
            </button>
            <button
              onClick={() => { setTaskTab('sell'); setActiveTab('sell') }}
              className={`px-4 py-2 text-sm font-bold rounded-t-lg transition-colors border-b-2 ${taskTab === 'sell' ? 'border-orange-500 text-orange-600' : 'border-transparent text-[#777587] hover:text-[#464555]'}`}>
              🏷️ Sold Tasks
            </button>
          </div>
