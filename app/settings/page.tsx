...
const DEPT_CONFIG = {
  LOGISTICS: { label: 'Logistics', icon: '📦', color: 'text-blue-400', bg: 'bg-blue-500/15', border: 'border-blue-500/30' },
  ACCOUNTING: { label: 'Accounting', icon: '💰', color: 'text-amber-400', bg: 'bg-amber-500/15', border: 'border-amber-500/30' },
  SALES: { label: 'Sales', icon: '🤝', color: 'text-green-400', bg: 'bg-green-500/15', border: 'border-green-500/30' },
} as const
...
export default function SettingsPage() {
  ...
  const inputClass = 'w-full bg-[#0d0d15] border border-white/10 rounded-xl px-4 py-3 text-white placeholder-slate-600 text-base focus:outline-none focus:border-blue-500'
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-8 flex-1 overflow-y-auto">
      ...
      {/* GreenAPI */}
      <section className="bg-[#16161f] rounded-2xl border border-white/10 p-6 mb-6">
        <h2 className="text-xl font-bold text-white mb-1">WhatsApp Integration</h2>
        <p className="text-slate-400 text-sm mb-5">GreenAPI credentials and auto-message schedule</p>
        <div className="flex flex-col gap-4">
          <div>
            <label className="text-sm text-slate-400 block mb-1.5">GreenAPI Instance ID</label>
          ...
          <div>
            <label className="text-sm text-slate-400 block mb-1.5">GreenAPI API Token</label>
          ...
          <div>
            <label className="text-sm text-slate-400 block mb-1.5">
              Auto-message Time <span className="text-slate-600">(Pakistan Standard Time, UTC+5)</span>
            </label>
      ...
      <div className="flex items-center gap-4 mb-8">
        <button onClick={saveSettings} disabled={saving}
          className="px-8 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-semibold text-base transition-colors disabled:opacity-50">
          {saving ? 'Saving...' : 'Save Settings'}
        </button>
        {savedMsg && (
          <span className={`text-sm font-medium ${savedMsg.startsWith('Error') ? 'text-red-400' : 'text-green-400'}`}>
            {savedMsg}
          </span>
        )}
      </div>
      ...
      <section className="bg-[#16161f] rounded-2xl border border-white/10 p-6">
        <div className="flex items-center justify-between mb-1">
          <h2 className="text-xl font-bold text-white">Team Members</h2>
          <span className="text-slate-400 text-sm bg-white/5 px-3 py-1 rounded-full">
          ...
          return (
            <div key={dept} className="mb-5">
              <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg mb-2 border ${cfg.bg} ${cfg.border}`}>
                ...
                <span className="text-slate-600 text-xs ml-auto">{deptMembers.length} member{deptMembers.length !== 1 ? 's' : ''}</span>
              </div>

              {deptMembers.length === 0 ? (
                <div className="text-slate-600 text-sm text-center py-3 bg-white/[0.02] rounded-xl border border-white/5">
                ...
                <div className="space-y-2">
                  {deptMembers.map(member => (
                    <div key={member.id}
                      className="flex items-center justify-between bg-[#0d0d15] rounded-xl px-4 py-3 border border-white/5">
                      <div className="flex items-center gap-3">
                        <div className={`w-9 h-9 rounded-full flex items-center justify-center text-white font-bold flex-shrink-0 ${
                          dept === 'LOGISTICS' ? 'bg-blue-600' : dept === 'ACCOUNTING' ? 'bg-amber-600' : 'bg-green-600'
                        }`}>
                          {member.name.charAt(0).toUpperCase()}
                        </div>
                        <div>
                          <p className="text-white font-medium text-base">{member.name}</p>
                          <p className="text-slate-500 text-sm">+{member.whatsapp_number}</p>
                        </div>
                      </div>

                      {deleteConfirm === member.id ? (
                        <div className="flex items-center gap-2">
                          <span className="text-slate-400 text-sm">Remove?</span>
                          ...
                          <button onClick={() => setDeleteConfirm(null)}
                            className="px-3 py-1 rounded-lg bg-white/10 text-slate-300 text-sm hover:bg-white/20 transition-colors">No</button>
                        </div>
                      ) : (
        ...
        {/* Add member form */}
        <form onSubmit={addMember} className="border-t border-white/10 pt-5 mt-2">
          <p className="text-slate-400 text-sm mb-3 font-medium">Add new member</p>
          ...
          <div className="flex gap-3">
            <input type="text" value={newMember.name}
              ...
              className="flex-1 bg-[#0d0d15] border border-white/10 rounded-xl px-4 py-2.5 text-white placeholder-slate-600 text-base focus:outline-none focus:border-blue-500" />
            ...
            <button type="submit" disabled={addingMember}
              className="px-5 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-semibold text-base transition-colors disabled:opacity-50 whitespace-nowrap">
              {addingMember ? '...' : 'Add'}
            </button>
          </div>
          {memberError && (
            <p className="text-red-400 text-sm mt-2 bg-red-400/10 rounded-lg px-3 py-2">{memberError}</p>
          )}
        </form>
      </section>

      {/* Webhook info */}
      <section className="bg-[#16161f] rounded-2xl border border-white/10 p-6 mt-6">
        <h2 className="text-xl font-bold text-white mb-1">Webhook URL</h2>
        <p className="text-slate-400 text-sm mb-4">Set this in GreenAPI → Instance settings → Webhooks</p>
        <div className="bg-[#0d0d15] rounded-xl px-4 py-3 font-mono text-sm text-blue-300 border border-white/10 select-all">
